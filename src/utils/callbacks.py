...
def create_and_save_tensorboard_callback(callbacks_dir, tensorboard_log_dir):
    ...
    logging.info(f"Tensor board logs are saved at {tb_callback_filepath}")
...
def create_and_save_checkpoint_callback(callbacks_dir, checkpoint_dir):
    ...
    checkpoint_callback=tf.keras.callbacks.ModelCheckpoint(checkpoint_file_path,save_best_only=True)
    ...
    logging.info(f"Check point call backs are saved at {ckpt_callbacks_filepath}")
